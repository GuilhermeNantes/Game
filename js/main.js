...
// OTIMIZAÇÃO: Reduz a resolução do mapa de sombras (maior ganho de GPU)
...
// OTIMIZAÇÃO: Pré-alocação do vetor de shake (evita alocação no loop)
const cameraShake = new THREE.Vector3(0, 0, 0); 

// Instâncias de Gerenciamento
let carMesh = null;
let trackMesh = null;
let carController = null;
let raceManager = new RaceManager(scene); 
...
// OTIMIZAÇÃO: Armazena malhas apenas do CHÃO para Raycasting vertical (y)
let groundMeshes = []; 
// NOVO: Armazena malhas de PAREDE para Raycasting horizontal (x, z)
let wallMeshes = []; 
// OTIMIZAÇÃO: Armazena a altura do carro (constante) para não recalcular a Box3 no loop
let carHeight = 0; 
...
function loadCar() {
    loader.load(
        'assets/models/1967_shelby_gt500.glb', 
        function(carGltf) {
...
            // OTIMIZAÇÃO: Calcula a altura do carro UMA VEZ
            carHeight = new THREE.Box3().setFromObject(carMesh).getSize(new THREE.Vector3()).y;
...
loader.load(
    'assets/models/cartoon_race_track_spielberg.glb', 
    function(gltf) {
        trackMesh = gltf.scene;
        setupShadows(trackMesh);
        
        // OTIMIZAÇÃO: Coleta malhas do chão e das paredes para Raycasting
        trackMesh.traverse((child) => {
            if (child.isMesh) {
                // FILTRO DO CHÃO (Malha principal da pista/terreno)
                if (child.name.toLowerCase().includes("track") || child.name.toLowerCase().includes("ground")) {
                    groundMeshes.push(child);
                // FILTRO DE PAREDE (Malhas de colisão invisíveis - use o prefixo 'COL_' no seu 3D)
                } else if (child.name.startsWith("COL_")) { 
                    wallMeshes.push(child);
                    // Oculta a malha de colisão e desliga sombras (para performance)
                    child.visible = false; 
                    child.castShadow = false;
                    child.receiveShadow = false;
                } else {
                    // Trata o resto dos objetos (decoreções, barreiras) como parte do chão para o Raycasting vertical
                    groundMeshes.push(child);
                }
            }
        });
...
        // NOVO: Passa o array de malhas de parede para o CarController
        carController.update(keyboard, wallMeshes);
        
        // 2. VERIFICA COLISÃO COM O CHÃO (Raycasting OTIMIZADO)
        raycaster.set(carMesh.position, downwardVector);
        // Usa groundMeshes (array menor) em vez de trackMesh.children (array potencialmente maior)
        const intersects = raycaster.intersectObjects(groundMeshes, true);
        
        if (intersects.length > 0) {
            const firstHit = intersects[0];
            // Usa a variável carHeight pré-calculada
...
        
        // Atualiza o vetor cameraShake pré-alocado
        cameraShake.set(
